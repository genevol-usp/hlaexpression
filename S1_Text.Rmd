...
RNA-seq constitutes a relatively new approach to quantify HLA expression. In order to place our novel findings in the context of well established methods, and because many researchers consider qPCR as a gold standard for HLA expression, we provide an in-depth comparison of RNA-seq with previous qPCR studies. We compare two aspects: (1) the quantification of the HLA lineage-level expression in RNA-seq and qPCR; (2) the influence of regulatory SNPs, previously documented in qPCR studies, in shaping expression levels in our RNA-seq study.
...
To compare our eQTL mapping results to the findings of previous studies which identified SNPs with a regulatory effect on HLA expression, we  used different methods to test whether tese SNP were statistically correlated (i.e., in linkage disequilibrium), and if these effects were independent.
...
Existing qPCR findings for the association of SNPs and HLA expression provide an important reference for any study on HLA expression. In our Table S3, we assessed the degree of independence of the eQTLs we mapped with respect to previously reported regulatory SNPs, including those from qPCR studies, but only showed results for the significant associations according to the RTC test. Here we dedicate more space to a comparison with previous SNPs shown to have regulatory role on HLA (Kulkarni et al, 2011; Thomas et al, 2012; Fairfax et al, 2012; Raj et al, 2016; Ou et al, 2019). We evaluate independence using D', $r^2$ and the Relative Trait Concordance (RTC) score (Nica et al, 2010; Delaneau et al, 2018). The RTC score tests if different eQTLs mark the same biological signal (Nica et al, 2010).
...
The eQTLs we mapped for *HLA-C* are very highly correlated at the haplotypic level with the variant described by Kulkarni et al (2011), which was shown to regulate *HLA-C* expression via a miRNA binding site at the 3’UTR. The minor allele frequencies of our variants are lower than that of the SNP in Kulkarni et al (2011), and the variation at our SNPs is completely nested within that of the previously validated SNP, i.e. the allele present in our SNPs predicts the allele in the previous SNP, but the opposite is not true. The RTC test, which is designed to formally evaluate if all variation explained by a SNP can be accounted for by another one, suggests that accounting for the previously validated SNP does not erase the signal of the SNPs identified in our data. Thus, despite the strong haplotypic association, the SNPs we report seem to be driving the biological signal in our sample. Further, two independent eQTLs we identified for HLA-C likely share the biological signals with one eQTL mapped in the GEUVADIS study and the eQTL mapped by Fairfax et al (2012). 

The high haplotypic association is also seen for the rank 1 eQTL which we mapped for *HLA-DQB1*, and a variant reported by Raj et al (2016) as part of a super-enhancer which regulates *HLA-DQA1*, *HLA-DQB1* and *HLA-DRB1*. For HLA-DP we also found  a strong association, both in terms of D’ and $r^2$, between our SNPs and those previously validated for *HLA-DPB1* (Thomas et al, 2012) and *HLA-DPA1* (Ou et al, 2019). However, the RTC test suggest that, although the allele in one SNP predicts the allele in previously validated SNP, accounting for the previous SNP does not remove the signal of the SNP we mapped (Table 3).
...
"./geuvadis_reanalysis/eqtl_mapping/transcriptomemapping/hla_personalized/rtc/top_previous_qtls/validated_variants/validated_variants_results.csv" %>%
    read_csv() %>%
    filter(d_prime > .65) %>%
    group_by(validated_var) %>%
    ungroup() %>%
    mutate(previous_snp = paste0(validated_var, " (", study, ")")) %>%
    select(Gene = 1, rank = 2, eQTL = 3, `Reg-SNP (study)` = previous_snp,  
           `r2` = 6, `D'` = 7, `AF (eQTL)` = 8, `AF (Reg-SNP)` = 9, RTC = 10) %>%
    kable(digits = 2) %>%
...
However, by exploring a larger catalog of previous regulatory SNPs, we show that the majority of the eQTLs we mapped likely share a biological signal with SNPs identified in previous studies (see S3 and S5 Tables).
...
[Thomas R et al. A Novel Variant Marking HLA-DP Expression Levels Predicts Recovery from Hepatitis B Virus Infection. Journal of Virology. 2012;86(12).](https://doi.org/10.1128/JVI.00406-12)
