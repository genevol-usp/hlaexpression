...
```{r}
include_graphics("./plots/expression_boxplot.png")
```
...
The plots don't show the whole data, but only the overlap between the two 
datasets (although the totality of data was used to estimate/impute expression 
on both datasets).

```{r}
include_graphics('./plots/lineages.png')
```
