devtools::load_all("/home/vitor/hlatools")

doMC::registerDoMC(40)

# Functions --------------------------------------------------------------------

make_index <- function(loci, infer_missing = TRUE) {

  nuc_paths <- paste0("/home/vitor/IMGTHLA/alignments/", loci, "_nuc.txt")

  if (infer_missing) {
    alignments <- 
      purrr::map_df(nuc_paths, ~hla_read_alignment(., omit = "N")) %>%
      dplyr::mutate(locus = sub("^([^*]+).+$", "\\1", allele)) %>%
      dplyr::select(locus, allele, cds) %>%
      dplyr::group_by(locus) %>%
      dplyr::filter(!all(grepl("\\*", cds))) %>%
      dplyr::ungroup() %>%
      split(.$locus) %>%
      purrr::map_df(~hla_infer(., .parallel = TRUE)) %>%
      dplyr::select(-locus)
  } else {
    alignments <-
      nuc_paths %>%
      purrr::map_df(~hla_read_alignment(., omit = "N", rm_incomplete = TRUE))
  }

  alignments %>%
  dplyr::mutate(allele3f = ifelse(grepl("_s\\d+$", allele), allele, 
				  hla_trimnames(allele, 3)),
		cds = hla_format_sequence(cds)) %>%
  dplyr::group_by(cds) %>%
  dplyr::summarize(allele = paste(allele, collapse = "/"), 
		   allele3f = paste(unique(allele3f), collapse = "/")) %>%
  dplyr::group_by(allele3f) %>%
  dplyr::mutate(n = n()) %>%
  dplyr::ungroup() %>%
  dplyr::mutate(allele = ifelse(n > 1L, allele, allele3f)) %>%
  dplyr::select(allele, cds) %>%
  dplyr::arrange(allele)
}

# ------------------------------------------------------------------------------

main_loci <- c("A", "B", "C", "DQA1", "DQB1", "DRB")

other_loci <-
  list.files("/home/vitor/IMGTHLA/alignments/", pattern = "_nuc\\.txt") %>% 
  strsplit("_") %>% 
  purrr::map_chr(1) %>%
  .[! . %in% c(main_loci, "ClassI", "ClassII")]

index <- 
  dplyr::bind_rows(make_index(main_loci, infer_missing = TRUE),
		   make_index(other_loci, infer_missing = FALSE)) %>%
  dplyr::mutate(allele = paste0("IMGT_", allele)) %>%
  split(.$allele) %>% 
  purrr::map("cds")

indexDNA <- Biostrings::DNAStringSet(unlist(index))

Biostrings::writeXStringSet(indexDNA, "./imgt_index.fa")	    
