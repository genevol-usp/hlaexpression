...
typings <- plyr::ldply(thresholds,
		       function(th) hla_genotype_dt(imgt_quants, 0) %>%
			   hla_apply_zigosity_threshold(th),
		       .id = "th")
...
best_th %>%
    ...
    left_join(accuracies, by = c("th", "locus")) %>%
    select(-th_average) %>%
    drop_na() %>%
    write_tsv("./genotyping_accuracies.tsv")

write_tsv(out_df, "./quantifications_genos/processed_imgt_quants.tsv")
