...
abundances_dt <- 
    rbindlist(abundances_list, idcol = "subject"
	    ...
	    ][, fpkm := counts_to_fpkm(NumReads, EffectiveLength), by = subject
	    ][, .(subject = name, target_id = Name, fpkm)]
...
gene_dt <- 
    abundances_dt[gene_set, on = .(target_id), nomatch = 0L
...
fwrite(gene_dt_wide, "./quantifications_imgt_fpkm_expressed90%.csv")
