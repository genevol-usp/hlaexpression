...
```{r}
include_graphics("./simulation/plots/ref_methods.png")
```

When we compare the estimated counts to the ground truth, we observe large deviation.
Genes in the lower right corner are mainly HLA-DRB5, RN7SL1 and SNORD3C, which are non-coding. This means that we should do some filtering on the annotations, as Manolis group and many other people advise.

```{r}
include_graphics("./simulation/plots/ref_methods_groundTruth.png")
```
...
```{r}
read_tsv("./geuvadis_reanalysis/eqtl_mapping/transcriptomemapping/hla_personalized/rtc/conventional_qtls/results.tsv") %>%
    kable(digits = 2)
```
